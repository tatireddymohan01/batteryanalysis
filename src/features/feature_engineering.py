...
class FeatureEngineer:
    ...
    def create_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "cycle_rate" in derived_features_config:
            df_features["cycle_rate"] = df_features["cycles"] / (
                df_features["usage_hours"] + 1
            )
        ...
        if "cumulative_stress" in derived_features_config:
            ...
            voltage_stress = (
                np.maximum(df_features["voltage_max"] - 4.1, 0) / 0.3
            )  # Normalized
            ...
            df_features["cumulative_stress"] = (
                ...
                * (
                    0.4 * temp_stress
                    + 0.3 * voltage_stress
                    + 0.3 * np.maximum(c_rate_stress, 0)
                )
            ) / 100  # Normalize
        ...
        if "temperature_humidity_interaction" in derived_features_config:
            ...
            humidity_stress = (
                np.abs(df_features["humidity"] - 50) / 50
            )  # Distance from optimal
            temp_stress_norm = (
                np.maximum(df_features["temperature"] - 25, 0) / 35
            )  # Normalized
        ...
        logger.info(
            f"Created {len(df_features.columns) - len(df.columns)} derived features"
        )

        return df_features

    def create_polynomial_features(
        self, df: pd.DataFrame, degree: int = None, fit: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if fit:
            ...
            if self.polynomial_features is None:
                raise ValueError(
                    "Polynomial features not fitted. Call with fit=True first."
                )

            poly_features = self.polynomial_features.transform(df[features_to_expand])
    ...
    def create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "temperature" in df.columns and "c_rate" in df.columns:
            df_interact["temp_crate_interaction"] = df["temperature"] * df["c_rate"]
        ...
        if "voltage_max" in df.columns and "c_rate" in df.columns:
            df_interact["voltage_crate_interaction"] = df["voltage_max"] * df["c_rate"]
    ...
    def engineer_features(self, df: pd.DataFrame, fit: bool = True) -> pd.DataFrame:
        ...
        logger.info(
            f"Feature engineering complete. Output shape: {df_engineered.shape}"
        )
    ...
    def save_feature_engineer(
        self, filepath: str = "./models/saved/feature_engineer.pkl"
    ):
        """
