...
from utils import save_model, load_model, format_time, print_metrics_summary
...
class ModelTrainer:
    ...
    def __init__(self, config: dict = None):
        ...
        mlflow.set_tracking_uri(mlflow_config.get('tracking_uri', './models/experiments'))
        mlflow.set_experiment(mlflow_config.get('experiment_name', 'battery_degradation'))
            ...
            with mlflow.start_run(run_name=model_name):
                try:
                    ...
                    if tune_hyperparameters and params:
                        ...
                        if params and not any(isinstance(v, list) for v in params.values()):
                            mlflow.log_params(params)
                    
                    # Re-evaluate if we did hyperparameter tuning
                    if tune_hyperparameters and params and any(isinstance(v, list) for v in params.values()):
                        y_train_pred = model.predict(X_train)
                        y_val_pred = model.predict(X_val)
                    ...
                    if val_rmse < self.best_score:
                        ...
                        logger.info(f"New best model: {model_name} (RMSE: {val_rmse:.4f})")
    ...
    def save_best_model(
        self,
        save_path: str = "./models/saved/best_model.pkl"
    ) -> None:
        """
        Save the best model.
...
def main():
    ...
    evaluator.compare_models(
        comparison_metrics,
        save_path="./models/experiments/plots/model_comparison.png"
    )
