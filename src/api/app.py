...
class BatteryInput(BaseModel):
    ...
    cycles: int = Field(
        ..., description="Number of charge/discharge cycles", ge=0, le=5000
    )
    temperature: float = Field(
        ..., description="Operating temperature in °C", ge=-20, le=80
    )
...
def load_models():
    ...
    try:
        ...
        feature_engineer.polynomial_features = feature_engineer_state[
            "polynomial_features"
        ]
...
async def health_check():
    ...
    return {
        "status": (
            "healthy"
            if all([model_loaded, preprocessor_loaded, feature_engineer_loaded])
            else "unhealthy"
        ),
...
async def predict(input_data: BatteryInput):
    ...
    try:
        ...
        if feature_engineer.feature_names:
            missing_cols = set(feature_engineer.feature_names) - set(
        ...
        return {
            ...
            "confidence": (
                "High"
                if abs(confidence_interval["upper"] - confidence_interval["lower"]) < 5
                else "Medium"
            ),
