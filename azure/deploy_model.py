...
class AzureMLDeployer:
    ...
    def register_model(self, model_path: str, model_name: str = None) -> Model:
        ...
        if model_name is None:
            model_name = self.config.get('azure', {}).get('model_registry', {}).get('model_name', 'battery_soh_predictor')
    ...
    def create_endpoint(self, endpoint_name: str = None) -> ManagedOnlineEndpoint:
        ...
        if endpoint_name is None:
            endpoint_name = self.config.get('azure', {}).get('deployment', {}).get('endpoint_name', 'battery-prediction-endpoint')
    ...
    def create_deployment(
        ...
        if deployment_name is None:
            deployment_name = self.config.get('azure', {}).get('deployment', {}).get('deployment_name', 'battery-prediction-v1')
        ...
        try:
            created_deployment = self.ml_client.online_deployments.begin_create_or_update(deployment).result()
